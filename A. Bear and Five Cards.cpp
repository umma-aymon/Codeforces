...
int main()
{
    ...
    sort(a,a+5);
    if(a[4]==a[3]&&a[3]==a[2])
        return cout<<a[0]+a[1],0;
    if(a[0]!=a[1]&&a[1]!=a[2]&&a[2]!=a[3]&&a[3]!=a[4])
        return cout<<a[0]+a[1]+a[2]+a[3]+a[4],0;
    if(a[4]==a[3]&&a[3]!=a[2] && a[2]==a[1] &&a[1]!=a[0])
    {
        if((a[4]+a[3]+a[0])<(a[2]+a[1]+a[0]))
            return cout<<(a[4]+a[3]+a[0]),0;
        else
            return cout<<(a[1]+a[2]+a[0]),0;
    }
    if(a[4]==a[3]&&a[3]!=a[2])
    {
        if(a[4]+a[3]<(a[0]+a[1]+a[2]))
            return cout<<a[4]+a[3],0;
        else
            return cout<<a[0]+a[1]+a[2],0;
    }
    if(a[0]==a[1]&&a[1]==a[2])
        return cout<<a[3]+a[4],0;
    if(a[0]==a[1]&&a[1]!=a[2] && a[2]==a[3] &&a[3]!=a[4])
    {
        if((a[0]+a[1]+a[4])<(a[2]+a[3]+a[4]))
            return cout<<(a[4]+a[1]+a[0]),0;
        else
            return cout<<(a[3]+a[2]+a[4]),0;
    }
    if(a[0]==a[1])
        return cout<<a[2]+a[3]+a[4],0;
    else
        cout<<a[0]+a[4];
    return 0;
}
...
for(auto i:m)
    if(i.second>1)
    ans=min(ans,sum-i.first*min(3,i.second));
    cout<<ans;*/
